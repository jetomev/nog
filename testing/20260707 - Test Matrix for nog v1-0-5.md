# nog — v1.0.5 release test matrix
...
- [ ] **1.1** `$ nog --version` prints `nog 1.0.5`
- [ ] **1.2** `$ nog --help` lists all subcommands (`install`, `remove`, `update`, `search`, `pin`, `unlock`) and neither of the hidden `_debug-*` commands
- [ ] **1.3** `$ man nog` opens cleanly; header reads `nog v1.0.5`; **PRIVILEGES AND SUDO** section present
...
## 17. Split-PKGBUILD pkgbase coupling (v1.0.4 — Phase 7)
...
## 18. Candidate-fresh hold evaluation (v1.0.5 — Phase 8)

The 2026-07-06 stale-DB finding: holds were dated from `/var/lib/pacman/sync` (refreshed only at the post-report handoff) while candidates came from checkupdates' freshly-synced private dbpath — so first-sighting updates were dated from their PREDECESSOR's builddate and could skip holds entirely (`lib32-brotli`: "975 days past window" for a package built the day before).

### 18a. Fresh-snapshot path

Non-destructive when everything buckets Held (nog exits before handoff). Best run right after repos publish new packages.

- [ ] **18.1** `$ nog update` — for each **Ready** entry, `days past window` must be plausible: cross-check one with `tar -xOf /tmp/checkup-db-$(id -u)/sync/<repo>.db <pkg>-<ver>/desc | grep -A1 %BUILDDATE%` — elapsed-since-build minus the tier window must equal the displayed figure (±1 day for ceil rounding)
- [ ] **18.2** `$ nog update` — a package built **today or yesterday** (check Arch's package pages) appears in **Held** with `<window> - 1` or `<window>` days remaining, NEVER in Ready with a large past-window figure
- [ ] **18.3** Independent recomputation: extract builddates for ALL pending candidates from `/tmp/checkup-db-$(id -u)/sync/*.db` and verify every bucket + days figure matches nog's report (this is how the fix was verified pre-release on 2026-07-07: 22/22 exact match)
- [ ] **18.4** No `nog: warning — checkupdates DB not found` on a normal run (the snapshot the just-finished checkupdates call wrote must be found)

### 18b. Fallback path

- [ ] **18.5** Simulating the fallback is awkward because `CHECKUPDATES_DB` and `TMPDIR` are honored by checkupdates too (it would just recreate the dir). Practical simulation: run nog with a `TMPDIR` where checkupdates lacks write permission for the sync step, or point `CHECKUPDATES_DB` at a path that exists but has no `sync/` subdir. If neither is practical on this machine, mark N/A — the fallback arm is small, covered by code review, and its warning copy is asserted in 18.6
- [ ] **18.6** When the fallback triggers, BOTH warning lines print (`checkupdates DB not found` + `Hold windows may be dated from stale build dates`) and the run continues rather than aborting

### 18c. Candidate-version guard

- [ ] **18.7** `cargo test --release --locked` — 29/29 green, including `candidate_version_mismatch_returns_unknown`, `candidate_version_match_evaluates_normally`, `candidate_without_db_version_skips_guard`, `candidate_missing_from_map_is_unknown`
- [ ] **18.8** Unknown-bucket copy: when an Unknown appears, the prompt intro reads "have no usable build date in any sync DB" and mentions version-mismatched DB entries
- [ ] **18.9** AUR packages (e.g. `fresh-editor-bin`) still bucket Ready/Held from helper-provided dates — the guard must NOT push version-less AUR entries to Unknown

### 18d. Legacy surfaces unaffected

- [ ] **18.10** `$ nog _debug-hold <pkg>` and `$ nog _debug-dates <pkg>` still work (they deliberately read the system sync DB — inspection tools, documented legacy)
- [ ] **18.11** `$ nog search`, `install`, `remove`, `pin`, `unlock` behavior unchanged (they never used hold evaluation on candidates)
